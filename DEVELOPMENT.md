...
- Common variables to set:
  ...
  - DUCKDNS_DOMAIN / DUCKDNS_TOKEN (only if you use the container nginx/certbot setup)
...
```
Caveat: the full docker setup expects DUCKDNS_DOMAIN and other environment variables for nginx/certbot; check .env under example.env and edit accordingly.
