...
import { sharedButtonColorHoverVariants, sharedButtonColorVariants } from "./theme/colorVariants";
...
function ButtonColored({
  ...
  return (
    <Comp
      data-slot="button"
      className={cn(buttonVariants({ color, hover: hover ? hover : color, variant: variant, size, className }))}
      {...props}
    />
  );
}

export { ButtonColored, buttonVariants };
