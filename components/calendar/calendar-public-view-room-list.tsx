...
export const RoomCategoryLayout = ({
  ...
  const breakpoints = isSidebarOpen
    ? "w-(--public-calendar-filter-w-col-1) xl:w-(--public-calendar-filter-w-col-2)"
    : "w-(--public-calendar-filter-w-col-1) xs:w-(--public-calendar-filter-w-col-2) sm:w-(--public-calendar-filter-w-col-1) lg:w-(--public-calendar-filter-w-col-2)";

  const gridBreakpoints = isSidebarOpen ? "xl:grid-cols-2" : "xs:grid-cols-2 sm:grid-cols-1 lg:grid-cols-2";
  ...
  return (
    <div className={`space-y-6 ${breakpoints}`}>
      ...
      {rows.map((row, index) => (
        <div key={index} className={`grid grid-cols ${gridBreakpoints} gap-4 w-full`}>
          <Group
            categories={[row.left]}
      ...
      {categories.map((category) => (
        <GroupHeader key={category} category={category}>
          {roomsByCategory[category]?.map((room) => {
            return <GroupRow key={room.roomId} room={room} checkedRooms={checkedRooms} toggleRoom={toggleRoom} />;
          })}
        </GroupHeader>
      ))}
    </div>
