...
export const RoomCategoryLayoutSkeleton = () => {
	const rooms: fakeRoomCategory[] = [...createCategories("GroupA", 8), ...createCategories("GroupB", 4), ...createCategories("GroupC", 2)];
	...
	return (
		<div className="space-y-6 w-(--public-calendar-filter-w-col-1) xs:w-(--public-calendar-filter-w-col-2) sm:w-(--public-calendar-filter-w-col-1) lg:w-(--public-calendar-filter-w-col-2) ">
			...
			{rows.map((row, index) => (
				<div
					key={index}
					className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-1 lg:grid-cols-2 gap-4 w-full "
				>
					{/* Left category */}
