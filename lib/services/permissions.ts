				...
				if (vars.assignRole) {
					if (idx === -1) {
						...
						const placeholder = {
							...
							roles: [{ roleId: Number(vars.roleId), role: vars.roleName }],
						} as IUserWithRoles;

						queryClient.setQueryData<IUserWithRoles[]>(key, [...next, placeholder]);
					} else {
						...
						if (!user.roles.some(r => String(r.roleId) === roleIdStr)) {
							user.roles.push({ roleId: Number(vars.roleId), role: vars.roleName });
						}
						next[idx] = user;
						queryClient.setQueryData<IUserWithRoles[]>(key, next);
					}
				} else {
					// Removing from this role list → remove user entirely
					const filtered = next.filter(u => String(u.userId) !== userIdStr);
					queryClient.setQueryData<IUserWithRoles[]>(key, filtered);
				}
			}
