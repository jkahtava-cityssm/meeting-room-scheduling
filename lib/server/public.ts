...
export async function fetchPublicEvents(startDate: string, endDate: string): Promise<{ data: PUBLIC_IEVENT[]; message: string }> {
...
export async function privateServerFetch<T>(url: string, contextMessage: string, options: RequestInit = {}): Promise<T> {
  return fetch(url, {
    ...options,
    headers: {
