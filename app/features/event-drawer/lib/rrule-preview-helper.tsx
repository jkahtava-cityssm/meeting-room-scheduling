...
export function getRRuleData({ startDate, values }: { startDate: string; values: RRuleFieldValues }): Promise<{
	...
	if (!RRuleOptions) {
		return Promise.resolve({ RRuleText, ruleString: "", ruleDescription: "", firstDate: "", lastDate: "", count: 0, localDates: [] });
	}
		...
		worker.onmessage = (
			...
			try {
				...
				const result = {
					...
					ruleDescription: response.data.ruleDescription,
...
function createRRule(startDate: string, values: RRuleFieldValues) {
	...
	const weekdayArray = getWeekdayArray(repeatingPattern, values.weekdays, values.monthWeekdayValue, values.yearWeekdayValue);
	...
	if ((durationType === "count" && count === 0) || (durationType === "until" && IsDateLessThenEqual(parsedStartDate, convertedEndDate))) {
		return;
	}
...
function getRepeatingPatternValue(repeatingType: string, dailyPattern: string, monthlyPattern: string, yearlyPattern: string) {
	const pattern =
		repeatingType === "daily"
			? dailyPattern
...
function getWeekdayArray(repeatingPattern: string, weekdays: string[], monthWeekdayValue: string, yearWeekdayValue: string) {
