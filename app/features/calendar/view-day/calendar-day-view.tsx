...
export function CalendarDayView({ date, userId }: { date: Date; userId?: string }) {
  ...
  return (
    <div className="flex flex-1 min-h-0">
      <div className={cn('flex flex-col min-h-0  min-w-0 transition-[width] duration-600 ease-in-out flex-1')}>
        {isMounting ? (
          <>
            ...
            <CalendarScrollContainerSkeleton hours={fallbackHours} totalColumns={visibleRooms ? visibleRooms.length : 10} />
          </>
        ) : (
          <>
            <DayViewDayHeader currentDate={date} />
      ...
      <CalendarDayColumnCalendar date={date} isLoading={isLoading} events={events || []} view={'day'}></CalendarDayColumnCalendar>
    </div>
  );
}
