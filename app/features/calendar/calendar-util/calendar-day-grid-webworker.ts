...
import { areIntervalsOverlapping, differenceInMinutes, endOfDay, format, isSameDay, isToday, startOfDay } from "date-fns";
...
async function processDayEvents(dayData: IDayGridMessage): Promise<IDayGridResponse> {
	...
	selectedRooms.forEach(roomId => {
		const events = filterEventsByRoom(dailyEvents, roomId).sort((a, b) => new Date(a.startDate).getTime() - new Date(b.startDate).getTime());
	...
	for (const [roomId, events] of eventsByRoom.entries()) {
		...
		groupedEvents.forEach((currentGroup, groupIndex) => {
			currentGroup.forEach((currentEvent, eventIndex) => {
				...
				const blockStyle = calculateEventBlockStyle(currentEvent, currentDate, groupIndex, groupedEvents.length, hasOverlap, {
