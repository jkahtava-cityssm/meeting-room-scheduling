...
self.onmessage = async (event: MessageEvent<ICalendarProcessData>) => {
	...
	try {
		...
		const viewBounds = calculateViewBoundaries(visibleHours, [...multiDayEvents, ...recurringEvents] as IEvent[], range.startDate, range.endDate);
		...
		const filteredByStatus = excludeStatusKeys && excludeStatusKeys.length > 0 ? filterEventsByStatus(filtered, excludeStatusKeys) : filtered;
