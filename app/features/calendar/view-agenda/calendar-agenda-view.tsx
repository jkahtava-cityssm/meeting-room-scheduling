...
export function CalendarAgendaView({ date, userId }: { date: Date; userId?: string }) {
  ...
  const { result, isLoading, error } = usePrivateCalendarEvents('AGENDA', date, visibleHours, userId, roomIds, selectedStatusKeys);
  ...
  const eventsToRender = useMemo(() => {
    ...
    return result?.data.sortedEvents.filter((room) => selectedRoomIds.includes('-1') || selectedRoomIds.includes(String(room.roomId)));
  ...
  return (
    <>
      <div className="flex flex-1 min-h-0">
        ...
        <CalendarDayColumnCalendar date={date} isLoading={isLoading} events={eventsToRender} view={'agenda'}></CalendarDayColumnCalendar>
      </div>
    </>
  );
}
