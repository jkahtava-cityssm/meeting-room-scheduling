...
function getCurrentEvents(events: IEventSingleRoom[], date: Date) {
  const now = getDateAsOfNow(date);
  return (
    events.filter(
      (event) => isWithinInterval(now, { start: event.startDate, end: event.endDate }) && event.status.key === ('APPROVED' as TStatusKey),
    ) || null
  );
}
...
export function CalendarDayColumnCurrentEvents({ events, date }: { events: IEventSingleRoom[]; date: Date }) {
  const [currentTime, setCurrentTime] = useState(getLastFiveMinuteInterval(getDateAsOfNow(date)));
  const [currentEvents, setCurrentEvents] = useState(getCurrentEvents(events, getLastFiveMinuteInterval(getDateAsOfNow(date))));
    ...
    <div className="flex flex-1 flex-col overflow-hidden space-y-3">
      ...
      {currentEvents.length === 0 && (
        <div className="flex-1 px-4 pt-4">
          <p className="p-4 text-center text-sm italic text-muted-foreground">No events are scheduled for this time and day.</p>
        </div>
      )}
                ...
                return (
                  <div key={event.eventId + '-' + index} className="space-y-1.5">
                    ...
                    <div className="flex items-center gap-1.5 text-muted-foreground">
                      <DynamicIcon name={event.roomIcon as IconName} color={event.roomColor as TColors} className="size-3.5" />
                      <BadgeColored color={event.roomColor as TColors}>{event.roomName}</BadgeColored>
                    </div>
