  ...
  .superRefine((data, ctx) => {
    ...
    if (data.repeatingType === "monthly") {
      ...
      if (data.monthlyPattern === "patternInMonth") {
        if (!data.monthPeriodValue)
          ctx.addIssue({ code: "custom", path: ["monthPeriodValue"], message: "Select period" });
        if (!data.monthWeekdayValue)
          ctx.addIssue({ code: "custom", path: ["monthWeekdayValue"], message: "Select weekday" });
      }
    }
  });
// --- Step 1 Schema ---
